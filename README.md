...
Kick emotes use their structured webhook metadata, and native Twitch emotes use the ordered fragments in `channel.chat.message`. For `v0.1.1-beta`, normal interactive rendering is deliberately text-only: Kick emotes display their provider-supplied readable names such as `:ppJedi:`, Twitch native emotes display names such as `Kappa`, and no emotes are submitted to Überzug++ or another graphical terminal backend. Mixed Kick and Twitch histories therefore have no external graphical surfaces. Browser-extension emotes from BetterTTV, FrankerFaceZ, or 7TV are ordinary text to Twitch EventSub and have no Twitch emote ID or Twitch CDN asset, so Streamchat also leaves those names readable rather than inventing provider metadata.
...
The provider parsers, emote IDs and metadata, Twitch CDN URL generation, provider-separated cache/download pipeline, and graphical controller/backend implementation remain in the codebase for future work. The existing `emotes` configuration is retained for compatibility, but graphical presentation is temporarily deferred regardless of mode while the terminal scrolling/reflow renderer is overhauled for a future beta. Normal interactive runs do not start the emote controller, download assets, touch the provider cache directories, or create graphical placements. Existing cache data under `${XDG_CACHE_HOME:-$HOME/.cache}/streamchat/emotes/{kick,twitch}/` is not removed. Emote presentation never rewrites or deletes archived message data.

Twitch fragment positions are converted to Streamchat's inclusive rune ranges, so CJK and emoji surrounding an emote do not shift its textual location. No per-message Helix lookup or scraped image URL is used. The preserved Twitch asset resolver uses only the documented CDN template `https://static-cdn.jtvnw.net/emoticons/v2/{id}/{format}/dark/3.0`, selecting `animated` only when EventSub advertises it and otherwise using `static`. This resolver and Kick's existing asset metadata are dormant during normal interactive presentation in this beta.

The optional graphical implementation and its sanitized `emotes.debug` diagnostics remain available to its focused infrastructure tests, but a normal `v0.1.1-beta` interactive run does not start that implementation and therefore produces no placement/cache activity. Graphical emote presentation is deferred until a future beta with a proper terminal scrolling/reflow overhaul. Role badges also remain the current compact textual markers; graphical role badges are not part of this beta.
