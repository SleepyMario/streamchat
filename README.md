...
Kick emotes use their structured webhook metadata, and native Twitch emotes use the ordered fragments in `channel.chat.message`. Streamchat also loads the global and current-channel BetterTTV and 7TV catalogues when Twitch connects, then recognizes exact whitespace-delimited names in Twitch messages. This catalogue lookup is best-effort and asynchronous: a slow or unavailable third-party service never delays or breaks Twitch chat, and an unavailable name simply remains readable text. Native Twitch ranges always take priority over third-party names. FrankerFaceZ names remain ordinary text for now.
