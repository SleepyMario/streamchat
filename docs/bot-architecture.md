...
YouTube bot replies use a separate Google/YouTube channel authorization stored under `bot.youtube`. `streamchat setup youtube-bot` deliberately requests the broad `youtube` account-management scope rather than a least-privilege grant; that authorization can permit destructive content operations through other YouTube API calls. The 3.8 implementation confines its use to resolving the bot channel identity and inserting command replies. Incoming relay messages carry the broadcaster video ID, so replies resolve and target that exact live chat even though the bot identity does not own the broadcast. The primary YouTube credentials and video target remain unchanged.
