...
Twitch bot replies may use a dedicated account instead of the broadcaster identity. `streamchat setup twitch-bot` authorizes that account with only `user:read:chat` and `user:write:chat`, resolves its Twitch user ID/login, and stores it under `bot.twitch` without replacing the primary channel-owner authorization. The channel target continues to come from `twitch.channel`. The existing Twitch application may be reused; its Client ID and secret are copied into the dedicated account record so token refresh remains self-contained. Production configuration must remain mode `0600` and must never be printed without redaction. The fixed, case-insensitive commands are `!commands`, `!language`, and `!gender`; all three enter the command-only journal when executed. Duplicate suppression is tracked independently per platform, channel and command. The same command may run again after the configured cooldown (60 seconds by default) or after four ordinary intervening chat messages, while a different command runs immediately on Twitch and Kick and alert events never enter this command cooldown. YouTube additionally serializes bot replies across commands and alerts because the platform may silently omit API-accepted messages posted too close together.
...
YouTube bot replies use a separate Google/YouTube channel authorization stored under `bot.youtube`. `streamchat setup youtube-bot` deliberately requests the broad `youtube` account-management scope rather than a least-privilege grant; that authorization can permit destructive content operations through other YouTube API calls. Streamchat confines its use to resolving the bot channel identity and inserting command and 4.0 alert replies. Incoming relay messages carry the broadcaster video ID, so replies resolve and target that exact live chat even though the bot identity does not own the broadcast. The primary YouTube credentials and video target remain unchanged. New memberships, aggregate membership-gifting purchases, Super Chats and Super Stickers map to the same shared subscription, gift-subscription and donation events used by Twitch and Kick. The aggregate gifting event carries the buyer's name and YouTube's gift count; individual gift-recipient and membership-milestone events remain visible and archived but deliberately do not trigger duplicate or misleading purchase acknowledgements.
...
Streamchat 3.4 receives basic Twitch follow, new-subscription and aggregate gift-subscription EventSub notifications and recognizes Bits in structured Twitch chat events. These events enter the normal Streamchat relay/archive once, and the dedicated Twitch bot posts a fixed, simple acknowledgement in Twitch chat. Aggregate gifts name the gifter and count when Twitch supplies them; anonymous gifts remain anonymous. Streamchat 4.0 completes the corresponding YouTube membership, aggregate membership-gift, Super Chat and Super Sticker path. Alert toggles and customizable wording remain deliberately deferred. Arbitrary rules, timed messages, message sequences, overlays, conversational AI, recording control, clips and PeerTube integration are **planned architecture**, not working features. Their dashboard controls remain disabled or absent until the corresponding adapter and runtime have real tests.

The completed path to the packaged 4.0 release is: 3.2 dedicated Twitch bot replies; 3.3 Twitch-specific role/badge emojis; 3.4 basic Twitch follow, subscription, gift-subscription and Bits alerts in chat plus the fixed `!language` command; 3.5 Kick bot replies with the same shared commands; 3.6 Kick-specific role/badge emojis; 3.7 basic Kick alerts in chat, including names and gift counts when Kick supplies them; 3.8 YouTube bot replies; 3.9 YouTube-specific role/badge emojis; 3.9.1 cross-platform command unification and `!gender`; 3.9.2 universal green broadcaster markers; and 4.0 YouTube membership, aggregate membership-gift, Super Chat and Super Sticker parity, including names and gift-membership counts when YouTube supplies them. Versions 3.2 through 3.9.2 are tested source checkpoints only. Docker, stable Gentoo, Ubuntu/Debian and Windows packaging resumes for 4.0. After 4.0, Streamchat enters maintenance mode for the following weeks apart from bug fixes and required provider changes.
