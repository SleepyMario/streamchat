...
| Twitch | A Twitch app Client ID and Client Secret, a browser-created user access/refresh token with exactly `user:read:chat user:write:chat channel:manage:broadcast moderator:manage:banned_users moderator:manage:chat_messages moderator:read:followers channel:read:subscriptions`, and a channel name or URL | [Twitch Developer Console](https://dev.twitch.tv/console/apps), [Twitch OAuth](https://dev.twitch.tv/docs/authentication/getting-tokens-oauth/), and [Twitch moderation](https://dev.twitch.tv/docs/chat/moderation/) | `streamchat setup twitch` |
...
Kick and Twitch chat both support the centrally implemented, case-insensitive `!commands` command and the fixed `!language` reply: `You can try Nederlands, English, Deutsch, 中文, 한국말, 日本語 and tiếng Việt on that fat Sleepy dude.` Cooldowns are independent per platform, channel and command, so different commands answer immediately. A duplicate command may answer again after the configured cooldown (60 seconds by default) or after four ordinary chat messages, whichever happens first. Suppressed duplicate commands do not themselves advance the message counter. The 3.8 YouTube-bot checkpoint extends these shared commands to YouTube; 3.9.1 is the explicit cross-platform command-parity audit.
...
To enable server-side Twitch ingestion, run `streamchat setup twitch --config /etc/streamchat/config.json`, authorize the seven scopes listed in the Twitch setup table, and choose the channel. Write, channel-management, and moderation scopes are used only by the interactive client. The server resolves the channel with `GET /helix/users`; subscribes to `channel.chat.message` version 1, `channel.follow` version 2, `channel.subscribe` version 1, and `channel.subscription.gift` version 1; follows EventSub reconnect URLs without creating duplicate subscriptions; and keeps Kick/relay running if Twitch later stops with a terminal provider error.
...
Without `VERSION`, builds use an exact Git tag or a development value containing the commit date and hash. Version 3.2 remains the last packaged checkpoint; versions 3.3 through 3.9.1 are source checkpoints, and distribution packaging resumes for 4.0.
