...
To make bot replies come from a separate Twitch account, run `streamchat setup twitch-bot`. Sign in to Twitch as that bot account before approving the browser prompt. This second authorization requests only `user:read:chat` and `user:write:chat`, stores the resolved identity under `bot.twitch`, and leaves the broadcaster's `twitch` configuration unchanged. The same Twitch developer application can be reused. In 3.4, Twitch chat supports `!commands` and the fixed `!language` reply: `You can try Nederlands, English, Deutsch, 中文, 한국말, 日本語 and tiếng Việt on that fat Sleepy dude.` Both commands are case-insensitive. Cooldowns are independent per platform, channel and command, so different commands answer immediately. A duplicate command may answer again after the configured cooldown (60 seconds by default) or after four ordinary chat messages, whichever happens first. Suppressed duplicate commands do not themselves advance the message counter. The 3.5 Kick-bot and 3.8 YouTube-bot checkpoints will add the same two commands and behavior to their respective platforms.
...
Without `VERSION`, builds use an exact Git tag or a development value containing the commit date and hash. Version 3.2 remains the last packaged checkpoint; versions 3.3 through 3.9 are source checkpoints, and distribution packaging resumes for 4.0.
